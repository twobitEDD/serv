...
// TestUpgrade tests if an Serv node can be upgraded from one version to another.
...
// TestCLITxs executes different types of transactions against an Serv node
	...
	}{
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--fees=5000000000aserv",
					"--gas-prices=50000aserv",
				)
			},
			expPass:   false,
