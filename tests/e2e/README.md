...
for running end-to-end tests on Serv.
...
# version(s) of initial serv node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded serv node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.servd:/root/.servd'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' serv cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'serv_43970-1'
CHAIN_ID
```
...
1. Build a docker image for the serv target version
...
the test suite exports `/.servd` folder from the docker container
...
desired Serv version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older Serv version to the container,
...
to run different versions of serv containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/twobitedd/serv/tags).
...
```log
ok  	github.com/twobitedd/serv/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start serv node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/twobitedd/serv/v10/cmd/evmosd/root.go:243 +0x2ca
        ...
        main.main()
            github.com/twobitedd/serv/v10/cmd/evmosd/main.go:20 +0x45
