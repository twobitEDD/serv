...
import (
	...
	v9 "github.com/twobitedd/serv/v12/app/upgrades/v9_1"
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("aserv", res))
				...
				suite.Require().Equal(balanceBefore.AmountOf("aserv"), sdk.NewDecFromInt(res))
			},
			true,
		},
		{ //nolint:dupl
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expectedSuccess {
				for i := range v9.Accounts {
					...
					balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "aserv")
