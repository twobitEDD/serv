...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	servMeta := banktypes.Metadata{
		Description: "Base Denom for Serv Chain",
		Base:        utils.BaseDenom,
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.AservDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    utils.BaseDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.AservDenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert aserv", func() {
			// Register 'aserv' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'aserv' thru IBC
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.ServChain.GetContext(), servMeta)
			s.Require().NoError(err)

			aservInitialBalance := s.app.BankKeeper.GetBalance(s.ServChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send aserv from Serv to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisServ, s.ServChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			aservAfterBalance := s.app.BankKeeper.GetBalance(s.ServChain.GetContext(), receiverAcc, utils.BaseDenom)
			s.Require().Equal(aservInitialBalance.Amount.Sub(math.NewInt(amount)).Sub(sendAndReceiveMsgFee), aservAfterBalance.Amount)

			// check ibc aserv coins balance on Osmosis
			aservIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AservIbcdenom)
			s.Require().Equal(amount, aservIBCBalanceBefore.Amount.Int64())

			// 2. Send aserv IBC coins from Osmosis to Serv
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AservDenomtrace.Path, teststypes.AservDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisServ, s.IBCOsmosisChain, teststypes.AservIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc aserv coins balance on Osmosis - should be zero
			aservIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AservIbcdenom)
			s.Require().Equal(int64(0), aservIBCSenderFinalBalance.Amount.Int64())

			// check aserv balance after transfer - should be equal to initial balance
			aservFinalBalance := s.app.BankKeeper.GetBalance(s.ServChain.GetContext(), receiverAcc, utils.BaseDenom)

			totalFees := sendBackCoinsFee.Add(sendAndReceiveMsgFee)
			s.Require().Equal(aservInitialBalance.Amount.Sub(totalFees), aservFinalBalance.Amount)

			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.ServChain.GetContext(), receiverAcc, teststypes.AservIbcdenom)
	...
	Describe("Performing recovery with registered coin", func() {
		...
		It("should recover and not convert uosmo to tokens", func() {
			...
			// Send 'uosmo' to Osmosis address in Serv Chain (locked funds)
			// sender_addr == receiver_addr
			s.SendAndReceiveMessage(s.pathOsmosisServ, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
			timeout := uint64(s.ServChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
			err := s.pathOsmosisServ.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
			s.Require().NoError(err)
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			receiverInitialAservBalance := s.app.BankKeeper.GetBalance(s.ServChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// should trigger claims logic and send aserv coins from claims to receiver
			...
			receiverFinalAservBalance := s.app.BankKeeper.GetBalance(s.ServChain.GetContext(), receiverAcc, utils.BaseDenom)

			s.Require().Equal(receiverInitialAservBalance.Amount.Add(claimableAmount).Sub(sendBackCoinsFee), receiverFinalAservBalance.Amount)
		})
	})
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("registered coin", func() {
		...
		It("should timeout and reconvert coins", func() {
			...
			// need to update serv chain to prove missing ack
