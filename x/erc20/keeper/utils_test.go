...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	err = s.app.BankKeeper.MintCoins(suite.ServChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.ServChain.GetContext(), inflationtypes.ModuleName, suite.ServChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	coinServ = sdk.NewCoin(utils.BaseDenom, sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinServ)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/aserv") as input parameter.
