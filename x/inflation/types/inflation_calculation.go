...
func CalculateEpochMintProvision(
	...
	// Multiply epochMintProvision with power reduction (10^18 for serv) as the
	// calculation is based on `serv` and the issued tokens need to be given in
	// `aserv`
	epochProvision = epochProvision.Mul(sdk.NewDecFromInt(servtypes.PowerReduction))
