...
func (k *Keeper) WithChainID(ctx sdk.Context) {
	chainID, err := servtypes.ParseChainID(ctx.ChainID())
	if err != nil {
		panic(err)
	}
	...
	if !(chainID.Cmp(big.NewInt(9001)) == 0 || chainID.Cmp(big.NewInt(9000)) == 0 || chainID.Cmp(big.NewInt(43970)) == 0 || chainID.Cmp(big.NewInt(53970)) == 0) {
		panic("EVM only supports Serv chain identifiers (9000, 9001, 43970, or 53970 )")
	}
...
func (k *Keeper) GetAccountWithoutBalance(ctx sdk.Context, addr common.Address) *statedb.Account {
	...
	ethAcct, ok := acct.(servtypes.EthAccountI)
	if ok {
